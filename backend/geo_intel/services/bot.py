...
BOT_TOKEN = os.environ.get("GEO_BOT_TOKEN")
BASE_URL = os.environ.get("APP_BASE_URL", "https://tg-full-stack.preview.emergentagent.com")
...
<b>Команди:</b>
/radar_on — увімкнути сповіщення
/radar_off — вимкнути сповіщення
...
"""
    ...
    async def process_update(self, update: dict):
        ...
        if text.startswith("/start"):
            await self.handle_start(chat_id, user_id, username)
        elif text.startswith("/radar_on"):
            await self.handle_radar_on(chat_id, user_id)
        elif text.startswith("/radar_off"):
            await self.handle_radar_off(chat_id, user_id)
        elif text.startswith("/status"):
            await self.handle_status(chat_id, user_id)
        elif text.startswith("/test"):
            await self.handle_test(chat_id, user_id)
    
    async def run_polling(self):
        """Run bot with long polling"""
        if not self.token:
            logger.warning("GEO_BOT_TOKEN not set, bot disabled")
            return
